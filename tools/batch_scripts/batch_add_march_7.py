...
def batch_add_march_7():
    ...
    # ItemsList format: [(ItemName, Qty or "—", is_refill)]
    data = [
        ("PEREZ, MARICEL", "LEAD PERSON", [
            ("BALLPEN (BODY)", 0.0, False),
            ("WHITEBOARD (BODY) PENTLE PEN", 0.0, False),
            ("INK (WHITE BOARD)", 0.0, False),
            ("REFILL BALLPEN", 0.0, True),
            ("CLIPBOARD", 0.0, False)
        ]),
        ...
        ("DIAZ, NORMAN", "PILER", [("EARPLUG", 0.0, False)]),
        ("BERJA", "SIMIK OPERATOR", [("EARPLUG", 0.0, False)]),
        ("MATAS, MARK ANTHONY", "SIMIK OPERATOR", [("EARPLUG", 0.0, False)]),
        ("GELVERO JAY", "SIMIK OPERATOR", [("EARPLUG", 0.0, False)]),
        ("CAGANTES, ANDY", "SEAMER OPER", [("EARPLUG", 0.0, False)]),
        ("PALOMA JESSA", "SAUCE FINALIST", [("EARPLUG", 0.0, False)]),
    ]
    ...
    with SessionLocal() as session:
        try:
            ...
            for emp_name, role, items in data:
                ...
                for item_name_raw, qty_raw, is_refill in items:
